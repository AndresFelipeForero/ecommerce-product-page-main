...
export class MainPageComponent {
  ...
  ngOnInit() {
    this.subscribe = this._iproductService
      ...
      .subscribe((products) => {
        ...
        if (!products.meta.pagination.pageCount) {
          this.showProducts = [];
          return;
        }
        ...
        this.showProducts = products.data.map(({ id, attributes }: any) => {
          ...
          let hoverImage: string =
            attributes.image.data[1].attributes.formats.medium.url;
  ...
  chargePaginationState() {
    this.subscribePagination = this.activateRoute.queryParams.subscribe(
      (params) => {
        if (params['page']) {
